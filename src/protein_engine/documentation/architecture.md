...
def resolve(species: str) -> dict: ...
    # {"taxonomy_id": int, "resolved_name": str}

# retrieval/uniprot_client.py
def fetch_proteins(taxonomy_id: int) -> list[dict]: ...

# retrieval/ncbi_client.py
def fetch_proteins(taxonomy_id: int) -> list[dict]: ...   # fallback

# retrieval/pdb_client.py
def has_structure(accession: str) -> bool: ...

# ranking/scorer.py
def rank(proteins: list[dict], taxonomy_id: int, top_n: int = 10) -> list[dict]: ...

# sequence/validator.py
def validate(protein: dict) -> dict: ...   # {"ok": bool, "protein": dict, "issues": list[str]}

# sequence/exporter.py
def write(species: str, proteins: list[dict], excluded: list[dict]) -> Path: ...
